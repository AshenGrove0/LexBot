...
async def hello(interaction: discord.Interaction):
    ...
    with sqlite3.connect("history.db") as connection:
        ...
        cursor.execute("INSERT INTO history (command, user, datetime) VALUES(?, ?, ?);",
                       ("/hello", interaction.user.mention, current_time))
        connection.commit()
...
async def help(interaction: discord.Interaction):
    ...
    with sqlite3.connect("history.db") as connection:
        ...
        current_time = get_current_time()
        cursor.execute("INSERT INTO history (command, user, datetime) VALUES(?, ?, ?);",
                       ("/help", interaction.user.mention, current_time))
        connection.commit()
    ...
    with sqlite3.connect("history.db") as connection:
        ...
        try:
            history = cursor.execute(
                "SELECT * FROM history ORDER BY datetime DESC LIMIT ?;", (amount,))
            history = history.fetchall()
            history = tabulate.tabulate(
                history, headers=["Index", "Command", "User", "Timestamp"])
        except Exception as e:
            print(e)
            await interaction.response.send_message(f"The quantity provided was too large.", ephemeral=False)
        cursor.execute("INSERT INTO history (command, user, datetime) VALUES(?, ?, ?);",
                       (f"/history {amount}", interaction.user.mention, current_time))
        connection.commit()
...
async def define(interaction: discord.Interaction, word: str):
    ...
    with sqlite3.connect("history.db") as connection:
        ...
        if definition == -1:
            cursor.execute("INSERT INTO history (command, user, datetime) VALUES(?, ?, ?);",
                           (f"/define {word}", interaction.user.mention, current_time))
            connection.commit()
...
async def synonym(interaction: discord.Interaction, word: str):
    ...
    with sqlite3.connect("history.db") as connection:
        ...
        if synonyms == -1:
            cursor.execute("INSERT INTO history (command, user, datetime) VALUES(?, ?, ?);",
                           (f"/synonym {word}", interaction.user.mention, current_time))
            connection.commit()
...
async def info(interaction: discord.Interaction, wikipedia_arg: str):
    ...
    with sqlite3.connect("history.db") as connection:
        ...
        try:
            ...
            cursor.execute("INSERT INTO history (command, user, datetime) VALUES(?,?,?);",
                           (f"/info {wikipedia_arg}", interaction.user.mention, current_time))
            connection.commit()
...
async def translate(interaction: discord.Interaction, translate_arg: str, target_lang: str):
    ...
    with sqlite3.connect("history.db") as connection:
        ...
        translated_text = GoogleTranslator(
            source='auto', target=target_lang).translate(translate_arg)
        cursor.execute("INSERT INTO history (command, user, datetime) VALUES(?, ?, ?);", (
            f"/translate {translate_arg} {target_lang}", interaction.user.mention, current_time))
        connection.commit()
    ...
    with sqlite3.connect("history.db") as connection:
        ...
        for w in range(len(tagged)):
            if tagged[w][0] == word:
                ...
                cursor.execute("INSERT INTO history (command, user, datetime) VALUES(?, ?, ?);", (
                    f"/classify {word} {sentence}", interaction.user.mention, current_time))
                connection.commit()
...
async def stem(interaction: discord.Interaction, word: str):
    ...
    with sqlite3.connect("history.db") as connection:
        ...
        try:
            ...
            cursor.execute("INSERT INTO history (command, user, datetime) VALUES(?, ?, ?);",
                           (f"/stem {word}", interaction.user.mention, current_time))
            connection.commit()
...
async def analyze(interaction: discord.Interaction, sentence: str):
    ...
    with sqlite3.connect("history.db") as connection:
        ...
        cursor.execute("INSERT INTO history (command, user, datetime) VALUES(?, ?, ?);",
                       (f"/analyze {sentence}", interaction.user.mention, current_time))
        connection.commit()
        if result != "":
